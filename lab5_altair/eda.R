...
h2hData <- subset(pollData, select=c("question_id", "poll_id", "state", "pct",
                                      "candidate_party", "candidate_name","created_at_date",
                                      "start_dt", "end_dt"))
...
h2hData <- h2hData %>%
  ...
  mutate(max.pct = max(pct),
         winner = ifelse(max.pct==pct, as.character(candidate_party), NA)) 
...
ggplot(primaryData[primaryData$candidate_party=="DEM",], aes(x=start_dt, y=dem_lead, group=candidate_name,
